...
def test_valid_boli_address():
    from bolilib import is_valid_boli_address

    main = valid_boli_address()
    test = valid_boli_address('testnet')

    assert is_valid_boli_address(main) is True
    assert is_valid_boli_address(main, 'mainnet') is True
    assert is_valid_boli_address(main, 'testnet') is False

    assert is_valid_boli_address(test) is False
    assert is_valid_boli_address(test, 'mainnet') is False
    assert is_valid_boli_address(test, 'testnet') is True


def test_invalid_boli_address():
    from bolilib import is_valid_boli_address

    main = invalid_boli_address()
    test = invalid_boli_address('testnet')

    assert is_valid_boli_address(main) is False
    assert is_valid_boli_address(main, 'mainnet') is False
    assert is_valid_boli_address(main, 'testnet') is False

    assert is_valid_boli_address(test) is False
    assert is_valid_boli_address(test, 'mainnet') is False
    assert is_valid_boli_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from bolilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % bolilib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(bolilib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(bolilib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(bolilib.blocks_to_seconds(16616)) == 2612035
