...
def is_valid_boli_address(address, network='mainnet'):
    ...
    boli_version = 140 if network == 'testnet' else 50
    ...
    try:
        ...
        # rescue from exception, not a valid BolivarCoin address
        return False

    if (address_version != boli_version):
        return False
...
# shims 'til we can fix the bolid side
def SHIM_serialise_for_bolid(sentinel_hex):
    from models import BOLID_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = BOLID_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in bolid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # bolid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    bolid_hex = serialise(obj)
    return bolid_hex


# shims 'til we can fix the bolid side
def SHIM_deserialise_from_bolid(bolid_hex):
    from models import BOLID_GOVOBJ_TYPES

    # unpack
    obj = deserialise(bolid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('Bolivarcoin.conf')
