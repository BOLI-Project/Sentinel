...
def has_boli_conf():
    ...
    valid_boli_conf = False

    # ensure boli_conf exists & readable
    #
    # if not, print a message stating that BolivarCoin Core must be installed and
    # configured, including JSONRPC access in Bolivarcoin.conf
    try:
        f = io.open(config.boli_conf)
        valid_boli_conf = True
    except IOError as e:
        print(e)

    return valid_boli_conf
...
def main():
    ...
    if not has_boli_conf():
        print("BolivarCoin must be installed and configured, including JSONRPC access in Bolivarcoin.conf")
        sys.exit(1)
