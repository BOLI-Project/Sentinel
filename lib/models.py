...
class GovernanceObject(BaseModel):
    ...
    # sync bolid gobject list with our local relational DB backend
    @classmethod
    def sync(self, bolid):
        golist = bolid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_bolid(bolid, item)
    ...
    @classmethod
    def import_gobject_from_bolid(self, bolid, rec):
        import decimal
        import bolilib
        import inflection
        ...
        # shim/bolid conversion
        object_hex = bolilib.SHIM_deserialise_from_bolid(object_hex)
        objects = bolilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from bolid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from bolid, with every run
        ...
        # get/create, then sync payment amounts, etc. from bolid - bolid is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(bolid)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from bolid! %s" % e)
            govobj.vote_delete(bolid)
            return (govobj, None)
    ...
    def vote_delete(self, bolid):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(bolid, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, bolid, signal, outcome):
        import bolilib
        ...
        output = bolid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = bolilib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(bolid, signal)

    def sync_network_vote(self, bolid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = bolid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = BOLID_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 boli addr, non-multisig
            if not bolilib.is_valid_boli_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid BolivarCoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (bolilib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/BoliDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import bolilib
        obj_data = bolilib.SHIM_serialise_for_bolid(self.serialise())
    ...
    def prepare(self, bolid):
        try:
            object_hash = bolid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = BOLID_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not bolilib.is_valid_boli_address(addr, config.network):
    ...
    def hash(self):
        import bolilib
        return bolilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = BOLID_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, bolid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - bolid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, bolid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - bolid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, bolid):
        now = int(time.time())
        return (self.created_at < (now - bolid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, bolid):
        if self.is_expired(bolid):
            return False

        return True

    def is_deletable(self, bolid):
        if self.is_expired(bolid):
            return True
