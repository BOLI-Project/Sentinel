...
class BoliDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some bolid methods
    ...
    @classmethod
    def from_boli_conf(self, boli_dot_conf):
        from boli_config import BoliConfig
        config_text = BoliConfig.slurp_config_file(boli_dot_conf)
        creds = BoliConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = bolilib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = bolilib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = bolilib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_bolid_proto_version_with_sentinel_ping)
